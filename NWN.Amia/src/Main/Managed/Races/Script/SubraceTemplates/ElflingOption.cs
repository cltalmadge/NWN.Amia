...
{
    [ScriptName("race_init_elfng")]
    public class ElflingOption : IRunnableScript
    {
        public int Run(uint nwnObjectId)
        {
            ...
            {
                NWScript.SendMessageToPC(nwnObjectId, "Elfling only works with the Half Elf base race.");
                return 0;
            }
            ...
            NWScript.SetCreatureAppearanceType(nwnObjectId, NWScript.APPEARANCE_TYPE_HALFLING);
        ...
        {
            TemplateItem.SetSubRace(nwnObjectId, "Elfling");
            TemplateItem.SetDexMod(nwnObjectId, 1);
            TemplateItem.SetConMod(nwnObjectId, -1);
        }
    }
}